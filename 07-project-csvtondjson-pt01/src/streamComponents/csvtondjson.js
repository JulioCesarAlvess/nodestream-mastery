...
export default class CSVToNDJSON extends Transform {
  ...
  *#updateBuffer(chunk) {
    // it'll ensure if we got a chunk that is note completed
    // and doesnt have a breakline
    // will concat with the previous read chunk
    // 1st time = 01,
    // 2st time = ,rick,address\n
    ...
    while (breaklineIndex !== INDEX_NOT_FOUND) {
      ...
      const lineProcessIndex = breaklineIndex + BREAK_LINE_SYMBOL.length;
      const line = this.#buffer.subarray(0, lineProcessIndex);
      const lineData = line.toString();

      // I'll remove from the main buffer the data
      // we already processed!
      this.#buffer = this.#buffer.subarray(lineProcessIndex);
